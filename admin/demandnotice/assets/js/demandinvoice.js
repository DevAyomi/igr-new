            ...
            <div class='w-[50%]'>
              ...
              <div class="border border-2 p-2 w-full" style="border-color: #000 !important;">
                <p class=" text-sm"><strong class='fontBold'>BUSINESS TYPE:</strong> <span
                    class="text-xs">${demandInvoiceInfo.tax_business_type ? demandInvoiceInfo.tax_business_type : 'Nil'}</span></p>
      ...
      demandInvoiceInfo.revenue_heads.forEach((demandnot, i) => {
        invoiceContainer += `
          <tr>
            ...
            <td class='text-xs'>₦ ${parseFloat(demandnot.current_year_amount).toLocaleString('en-US', { minimumFractionDigits: 2, })}</td>
            <td class='text-xs'>₦ ${parseFloat(demandnot.previous_year_amount).toLocaleString('en-US', { minimumFractionDigits: 2, })}</td>
