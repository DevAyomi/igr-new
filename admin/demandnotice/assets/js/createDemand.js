  ...
  $("#moreRevItems").append(`
    <div class="revItems mb-4">
      ...
      <div class="d-flex gap-2">
        <div class="col-6">
          <div class="form-floating">
            <input type="text" class="form-control previous_year" />
        ...
        <div class="col-6">
          <div class="form-floating">
            <input type="number" class="form-control previous_year_amount" />
            <label for="amountP">Amount</label>
          </div>
        </div>
      </div>
    </div>  
...
async function generateTheInvoice(tax_numbber) {
  let dataToSend = {
    ...
    tax_office: "Main Tax Office",
    file_number: $("#file_number").val(),
    lga: "Auyo",
    description: "Annual Demand Notice",
    revenue_heads: [],
  };
  ...
  assessmentItems.forEach((item) => {
    ...
    const selectedOption = item.querySelector(".revenue_item").options[item.querySelector(".revenue_item").selectedIndex];
    let mda_id = selectedOption.getAttribute('data-mdaid')
    let current_year_amount = item.querySelector(".amount").value;
    let previous_year_date = item.querySelector(".previous_year").value;
    let previous_year_amount = item.querySelector(".previous_year_amount").value;

    let previousYearAmounto = parseFloat(previous_year_amount) === "" ? 0 : parseFloat(previous_year_amount)
    let totalAmount = parseFloat(current_year_amount) + previousYearAmounto
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status === "success") {
        $("#previewInvoice").attr(
          "href",
          `./demandinvoice.html?invoice_number=${data.invoice_number}`
        );
        nextPrev(4);
      } else {
        $("#msg_box").html(
