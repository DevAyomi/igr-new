...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const analyticsPermissions = [
    { id: 2, selector: "#revenueCollection-analysis" },
    { id: 3, selector: "#invoicingAnalytics" },
    { id: 4, selector: "#taxpayer-analytics" },
    { id: 63, selector: "#tax-types-analytics" },
    { id: 65, selector: "#payment-analytics" },
    { id: 66, selector: "#tcc-analytics" },
    { id: 67, selector: "#compliance-analytics" },
    { id: 69, selector: "#exemptions-analytics" },
    { id: 70, selector: "#notifications-analytics" },
    { id: 71, selector: "#system-usage-analytics" }
  ];

  let hasAnyPermission = false;

  analyticsPermissions.forEach(({ id, selector }) => {
    if (hasPermission(id)) {
  ...
  if (!hasAnyPermission) {
    $("#analyticsAccordion").html(noAccessHTML)
  }

  // DEMAND NOTICE

  if (hasPermission(54)) {
    $("#newDemandNoticeBtn").removeClass('d-none')
  }

  if (hasPermission(56)) {
    $("#downloadDemandBtn").removeClass('d-none')
    $("#downloadDemandBtn").addClass('d-flex')
  }

  // ESERVICES PERMISSIONS 
  if (hasPermission(62)) { // Approve Tax Filling
    $("#taxFilingActions").removeClass("d-none")
    $("#taxFilingActions").addClass("d-flex")
  }
