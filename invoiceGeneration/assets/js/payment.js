...
async function fetchInvoiceAmount() {
  ...
  $.ajax({
    ...
    success: function (data) {
      console.log(data);
      const response = data.data[objectoFill[checker]][0];
      $(".amountLabel").text(
        `₦ ${parseFloat(response?.amount_paid).toLocaleString()}`
      );
    },
...
paymentMethods.forEach((method) => {
  ...
  <label class="form-check-label bg-gray-100 text-center flex flex-col rounded-xl p-2" for="${
    method.id
  }">
    <input
    ...
    if (result.status === "success") {
      if (result.data[objectoFill[checker]].length > 0) {
        ...
        var handler = PaystackPop.setup({
          key: "pk_test_990410da873157f64e39790514b9512f777d07dc",
...
async function pushDataToDb() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (
        data.status === "success" &&
        data.data[objectoFill[checker]].length > 0 &&
        data.data[objectoFill[checker]][0].payment_status === "paid"
      ) {
        $("#msg_box").html(
