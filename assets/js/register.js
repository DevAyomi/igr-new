...
let verifyMedia = urlParams.get("verify");
let regType = urlParams.get("user");
if (verifyMedia == "phone") {
  document.getElementById("verificationMethod").innerHTML = "+2348067676767";
} else if (verifyMedia == "email") {
  document.getElementById("verificationMethod").innerHTML =
...
function selectCard(element) {
  ...
  selectedDataId = element.getAttribute("data-id");
}
...
function proceed() {
  ...
  if (selectedDataId === "individual") {
    window.location.href = `registerindv.html`;
  } else {
    window.location.href = `registercorp.html`;
  }
}
...
function proceedToVerification() {
  // Retrieve selected verification method
  const selectedMethod = document.querySelector(
    'input[name="verificationMethod"]:checked'
  ).value;

  // Redirect to the verification page
  window.location.href = `verification.html?verify=${selectedMethod}`;
}

function verifyOtp() {
  // TODO: Verify the otp logic here
